class Trip:  
    ...
    def generate_question(self):
        question = f"Create an example travel itinerary with start date {self.startDate} and end date {self.endDate}. "
        
        if self.interests:
            interests_formatted = ", ".join(self.interests)
            question += f"The trip should include the following interests: {interests_formatted}. "
        
        if self.countries:
            countries_formatted = ", ".join(self.countries)
            question += f"and visit the following countries: {countries_formatted}. "
        
        if self.isSustainable:
            question += "The itinerary should focus on sustainability, including eco-friendly activities, accommodations, and transportation options. "
        
        question += ("The response should be formatted as a JSON where each object contains the following details: "
